...
class Redis {
	...
	public function read($sessID) {
		...
		$_SESSION = json_decode($this->get_result,true);
	...
	public function write($sessID, $sessData) {
        ...
        $dd = json_decode($rdata,true);

        if ($dd != null){
        	$dd[$skey]=$_SESSION[$skey];
        	//array_push($dd, $_SESSION);
        }else{
        	$dd = $_SESSION;
        }
        
        if(is_int($expire) && $expire > 0) {
            $result = $this->handler->setex($sessID, $expire, json_encode($dd));
			$re = $result ? 'true' : 'false';
        }else{
            $result = $this->handler->set($sessID, json_encode($dd));
			$re = $result ? 'true' : 'false';
        }
        return $result;
	}
