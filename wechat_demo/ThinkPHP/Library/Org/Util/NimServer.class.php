...
class NimServer{
    ...
    const   CONTENT_TYPE_URLENCODED = "Content-Type:application/x-www-form-urlencoded;charset=utf-8";
    const   CONTENT_TYPE_MULTIPART  = "Content-Type:multipart/form-data;charset=utf-8";
     ...
     * @param  $isMultipart [是否是Multipart上传]
     * @return $请求返回结果(array)
     */
    public function postDataCurl($url,$data,$isMultipart=false){
        ...
        if($isMultipart){
            array_push($http_header, self::CONTENT_TYPE_MULTIPART);
            $postdata = $data;
        }else{
            array_push($http_header, self::CONTENT_TYPE_URLENCODED);
            $postdataArray = array();
    ...
    public function uploadMultiMsg($content,$type='0'){
        ...
        if($this->RequestType=='curl'){
            $result = $this->postDataCurl($url,$data,true);
        }else{
            $result = $this->postDataFsockopen($url,$data);
        }
        return $result;
